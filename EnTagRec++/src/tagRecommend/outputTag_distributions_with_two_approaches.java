...
public class outputTag_distributions_with_two_approaches {
	...
	static boolean termTagIndex = false;
	static final int crossNumber = 10;
	// return all topics
	static int infer_topK = 1000;
	static int termTagIndex_topK = 1000;
			...
			for (String testk : SingleComp) {
				...
				try {
					for (int i = 0; i < crossNumber; i++) {
						...
						outputQuery(currentTestDir+"\\after_testdata.cvs",queryListForTesting,"f");
						outputQuery(currentTestDir+"\\after_traindata.cvs",queryListForTraining,"f");
						queryListForTesting.clear();
						queryListForTraining.clear();
						System.gc();
						// evaluate
...
//						// bw.close();
	...
	public static void outputQuery(String path, ArrayList<Query> queryList, String approach) {
		try {
			BufferedWriter bw = new BufferedWriter(new FileWriter(path));
			if(approach.contains("b")){
			bw.write("LLDA\n");
			for (Query q : queryList) {
				if(q.getInferedTagsFromLLDA()==null || q.getInferedTagsFromLLDA().isEmpty())
					continue;
			...
			if(approach.contains("f")){
			bw.write("TermTagIndex\n");
			for (Query q : queryList) {
				if(q.getTermTagInferTags()==null || q.getTermTagInferTags().isEmpty())
					continue;
				StringBuilder sb = new StringBuilder();
				sb.append(q.query_id + ",");
