...
public class b_f_individule_precision {
		...
		for (int returnK : returnTopKs) {
			...
			for (int testk : testKs) {
				...
				for (String singleC : SingleComp) {
					...
					String resultOutput = TestDir + "\\recall_precision" + "@"
							+ returnTopK + "_"+singleC+"_" + infer_topK + "_"
					...
					try {
						for (int i = 0; i < crossNumber; i++) {
							 ...
							 * goldenSet.get(id).getTaglist(); int count = 0;
							 * for (String tag : tags) {
							 * 
