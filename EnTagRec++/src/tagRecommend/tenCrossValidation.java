...
public class tenCrossValidation {
	 ...
	 * @param args
	 */
	// turn on to generate the cross validation data
	static boolean generateCrossData = false;
	// turn on the llda to learn a model for each folder
	static boolean estimated = true;
	//turn on to get the extend network
	static boolean generateNetwork = false ;
	// query expansion or not
	static boolean expansion = false;
	// turn on llda inference component
	static boolean topicInfer = true;
	// turn on frequetist inference component
	static boolean termTagIndex = true;
	// set the number of folder for cross validation
	public static final int crossNumber = 10;
	//k parameter 
	static final int infer_topk = 70;
	static final int termTagIndex_topK = 50;
	// evaluate on top k tags. For example, set topK to 5, recall@5,
	...
	public static void main(String[] args) {
	   // set the sample data path
		//String root = "G:\\research\\tag_recommendation\\data_and_results\\AlltheFourDataset\\sample_data\\";
		...
		// the folder to store the results of 10-cross validation
		String TestDir = root + "testcase\\";
		// output of recalls
		String resultOutput = TestDir + "\\recall"+"_" + topK+".csv";
		
		// the folder store the pre-processed data
		...
		try {
			for (int i = 0; i < crossNumber; i++) {
				...
				//load golden set
				//BufferedWriter bw = new BufferedWriter(new FileWriter(TestDir+i+"\\tagInferred_result.txt"));
				...
				for (Query q : queryListForTesting) {
					...
					String content = TermTagIndexBuilder.readContentIntoOneLine(input);
					q.setText(content);
				...
				if(expansion){
					// generate network
					String graphInputForTesting = currentTestDir +"\\GraphresultForTesting.txt";
					String graphInputForTraining = currentTestDir + "\\GraphresultForTraining.txt";
					if (generateNetwork) {
						// output for training
				...
				//int[] a = new int[2];
				// used for get intermediate data
				...
				BufferedWriter bwLocal = new BufferedWriter(new FileWriter(
						currentTestDir + "\\recall_withoutTrain.csv"));
				
				// used for debugging
				/*for (Query q : queryList) {
					String id = q.id;
					bw.write(id+"\t");
					ArrayList<String> tags = goldenSet.get(id).getTaglist();
					int count = 0;
					for (String tag : tags) {
					...
					for(String tag : q.getTags()){
						bw.write(tag+"@"+ q.getInferedTags().get(tag).getWeight()+",");
					}
					bw.newLine();
				...
				LinearCombinationTrainerAndTester crossValidation = new LinearCombinationTrainerAndTester(queryListForTesting, goldenSet,topK);
				//HashMap<String, Double> recalls = crossValidation.performCrossValidation(10);
				
				//train a and b, for the linear combination, paras[3] and paras[4] for extension.
				double[] paras = crossValidation.trainTwoParameters(queryListForTraining,4);
				...
				System.out.println("iter"+i+"\t"+paras[0] + "\t" + paras[1] + "\t" +averageRecall);
				bwLocal.close();
				//bw.close();
