...
public class TestonKvary_recall_precision {
	...
	public static void main(String[] args) {
		...
		//String root = "G:\\research\\tag_recommendation\\folksonomy\\StackOverFlowForShaowei\\";
		...
		for (int returnK : returnTopKs) {
			...
			for (int testk : testKs) {
				...
				String resultOutput = TestDir + "\\ubf_recall_precision" + "@" + returnTopK
				...
				try {
					for (int i = 0; i < crossNumber; i++) {
						...
						IOManager.writeToLogFile(logFile,queryListForTesting,50);	
					...
					final_results.put(resultOutput, (double)recall_sum/(double)result_recall.size());
				} catch (IOException e) {
