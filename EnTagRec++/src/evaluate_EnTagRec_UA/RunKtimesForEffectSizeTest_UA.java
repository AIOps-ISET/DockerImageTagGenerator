...
public class RunKtimesForEffectSizeTest_UA {
	...
	public static void main(String[] args) throws IOException {
		...
		// String root = "E:\\shaowei\\freecode\\";
		 String root = "G:\\research\\tag_recommendation\\data_and_results\\AlltheFourDataset\\askubuntuSource\\";
		//String root = "G:\\research\\tag_recommendation\\data_and_results\\AlltheFourDataset\\appleSource\\";
		// String root =
		...
		BufferedWriter bwRepeat = new BufferedWriter(new FileWriter(root+"repeat_precision_recall_AU.csv"));
		bwRepeat.write("id,EnRecTagU_P@5,EnRecTagU_R@5,EnRecTagU_P@10,EnRecTagU_R@10,EnRecTagUA_P@5,EnRecTagUA_R@5,EnRecTagUA_P@10,EnRecTagUA_R@10\n");

		for (int repeatI = 0; repeatI < repeatN; repeatI++) {
					...
					for (int testk : testKs) {
						...
						try {
							for (int i = 0; i < crossNumber; i++) {
								...
								// the set storing the tags need to be excluded
								// in evaluation
								HashMap<String, List<String>> exclusiveTags = InferenceWithAssocRules
										.getExclusiveTags(goldenSet, givenTagN);
								;
								if (useAddiionalTags) {
									InferenceWithAssocRules inferAR = new InferenceWithAssocRules(
											currentTestDir + "\\", minsup,
											minconf, minlift);
									inferAR.initialize();
									inferAR.infer(queryListForTesting,
											goldenSet, givenTagN, debug);
