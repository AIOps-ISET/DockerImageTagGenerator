...
public class TestonKvary_recall_precision {
	...
	public static void main(String[] args) {
		...
		//String root = "W:\\research\\tag_recommendation\\data\\AlltheFourDataset\\askubuntuSource\\";
		 //String root =		 "F:\\shaowei\\research\\tag_recommendation\\folksonomy\\AlltheFourDataset\\appleSource\\";
		String root =		 "F:\\shaowei\\research\\tag_recommendation\\folksonomy\\AlltheFourDataset\\askubuntuSource\\";

		// String root ="F:\\shaowei\\research\\tag_recommendation\\folksonomy\\StackOverFlowForShaowei\\";
		String inputData = root + "dataset_origTag.csv";
		String TestDir = root + "testcase_OriginalTags\\";
		int[] testKs = {70};
		int[] returnTopKs = {5};
		for (int returnK : returnTopKs) {
			...
			for (int testk : testKs) {
				...
				String resultOutput = TestDir + "\\originalTag_fbu_recall_precision" + "@" + returnTopK
				...
				try {
					for (int i = 0; i < crossNumber; i++) {
						...
						HashMap<String, Double> precisions = crossValidation
								.calculatPrecision(paras, queryListForTesting);
						double sum = 0;
					...
					System.out.println("final average precision and recall:" + (double)precision_sum/(double)result_precision.size()
							+"\t"+ (double)recall_sum/(double)result_recall.size());
				} catch (IOException e) {
